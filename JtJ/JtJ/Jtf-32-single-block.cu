...
//核函数发生了错误
__global__ void jtf_cam( int* blocks)
{
	////分配共享内存
	...
	/*
	int4 jab= tex1Dfetch(tex_jtf_cam, threadIdx.x*2+threadIdx.y);
	int err= tex1Dfetch(tex_f_cam, tid);
	rx += jab.x * err;		ry += jab.y * err;
    rz += jab.z * err;  	rw += jab.w * err;
	*/
	...
	//	{
	//		//共享内存规约
	//		value[tid3] += value[tid3 + i/2];
	//		tid3 += threadsPerBlock;
	//	}  
	//	i /= 2;
	//}
	...
	if(threadIdx.x < 4)
        blocks[threadIdx.x+(threadIdx.y<<2)]=value[threadIdx.x+(threadIdx.y<<7)];
}
...
int main( void ) 
{
	...
	//核函数
	dim3 grid(1), block(threadsPerBlock,2);
	jtf_cam<<<grid, block>>>( dev_jtf );
